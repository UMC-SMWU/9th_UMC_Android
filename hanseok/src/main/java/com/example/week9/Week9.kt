...
import com.example.week9.Album
import com.example.week9.HomeFragment
import com.example.week9.LockerFragment
import com.example.week9.Song
import com.example.week9.SongActivity
import com.example.week9.databinding.ActivityWeek9Binding
import com.google.firebase.database.FirebaseDatabase

import kotlin.jvm.java

class Week9 : AppCompatActivity() {
    ...
    private fun initDummyDataAndPlayList() {
        ...
        if (isFirstLoading) {
            inputDummyDataToFirebase {
                ...
                Log.d("Week9", "Firebase 더미 데이터 생성 완료 및 플레이리스트 초기화 시작")
            }
        } else {
            initPlayList()
        }
    ...
    private fun inputDummyDataToFirebase(onComplete: () -> Unit) {
        ...
        rootRef.updateChildren(updates)
            .addOnSuccessListener {
                Log.d("Week9", "Firebase 더미 데이터 생성 성공!")
                onComplete()
            }
            .addOnFailureListener {
                Log.e("Week9", "Firebase 더미 데이터 생성 실패", it)
                onComplete()
            }
    }
    ...
    private fun setPlayer(song : Song){
        ...
        if (music == 0) {
            Log.e("Week9", "Resource not found: ${song.music}")
        } else {
    ...
    override fun onStart(){
        ...
        // 지금 플레이어에 뭔가 문제가 있는데 그거땜에 메인화면 실행에 문제가있어서 일단 이렇게 예외처리 했습니다
        if (songId != null && songId.isNotEmpty()) {
            database.getReference("songs").child(songId).get().addOnSuccessListener { snapshot ->
                ...
                if (currentSong != null) {
                    ...
                    if (nowPos != -1 && nowPos < songs.size) {
                        songs[nowPos] = currentSong
                        setPlayer(songs[nowPos])
                    } else {
                        Log.e("Week9 - onStart", "유효하지 않은 songId($songId) 또는 nowPos($nowPos)입니다.")
                    }
                }
            }
        }
    ...
    override fun onPause(){
        ...
        if (songs.isNotEmpty()) {
            ...
            database.getReference("songs").child(song.id).updateChildren(updates)
                .addOnSuccessListener {
                    Log.d("Week9", "onPause: 재생 상태 업데이트 성공")
                }
                .addOnFailureListener {
                    Log.e("Week9", "onPause: 재생 상태 업데이트 실패", it)
                }
        }
    }
